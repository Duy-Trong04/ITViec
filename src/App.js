...
function App() {
  ...
  useEffect(() => {
    fetch('https://6724d671c39fedae05b2efb7.mockapi.io/0306221384/TruongDuyTrong/job') // Thay API thật của bạn vào đây
      .then(response => response.json())
      .then(data => {
        // Giả lập format dữ liệu giống danh sách công việc ban đầu
        const formattedJobs = data.slice(0, 10).map(item => ({
          id: item.id,
          title: item.title,
          logo: item.logo,
